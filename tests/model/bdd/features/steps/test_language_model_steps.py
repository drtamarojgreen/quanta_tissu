...
def register_steps(runner):
    ...
    def when_generate_with_cache(context, count):
        ...
        generated_text = generate_text_helper(model, tokenizer, context['prompt'], int(count), method="greedy")
    ...
    def when_generate_for_rules(context):
        model, tokenizer = context['model'], context['tokenizer']
        context['raw_text'] = generate_text_helper(model, tokenizer, context['prompt'], 50, method="nucleus", top_p=0.9)
