...
void test_embedding_dimensions() {
    ...
    Matrix d_out = Matrix::random({5, (int)TestConfig::EmbedDim});

    layer.forward(input_tokens);
    layer.backward(d_out, input_tokens);

    auto* param = layer.parameters()[0];
    assert(param->value().shape() == param->grad().shape() && "Embedding grad shape mismatch");
...
void test_layernorm_dimensions() {
    ...
    Matrix input = Matrix::random({5, (int)TestConfig::EmbedDim});
    Matrix d_out = Matrix::random({5, (int)TestConfig::EmbedDim});
    ...
    for (auto* param : layer.parameters()) {
        assert(param->value().shape() == param->grad().shape() && "LayerNorm grad shape mismatch");
    }
...
void test_feedforward_dimensions() {
    ...
    for (auto* param : layer.parameters()) {
        assert(param->value().shape() == param->grad().shape() && "FeedForward grad shape mismatch");
    }
    std::cout << "    [PASSED] FeedForward layer dimensions are consistent." << std::endl;
}

void test_attention_dimensions() {
    std::cout << "  --- Testing ConfigurableAttention Layer Dimensions ---" << std::endl;
    ConfigurableAttention layer(TestConfig::EmbedDim, TestConfig::NumHeads, AttentionMode::STANDARD_MULTI_HEAD, 0);
    Matrix input = Matrix::random({1, 5, (int)TestConfig::EmbedDim});
    Matrix d_out = Matrix::random({1, 5, (int)TestConfig::EmbedDim});

    layer.forward(input);
    layer.backward(d_out);

    for (auto* param : layer.parameters()) {
        assert(param->value().shape() == param->grad().shape() && "Attention grad shape mismatch");
    }
    std::cout << "    [PASSED] ConfigurableAttention layer dimensions are consistent." << std::endl;
}
