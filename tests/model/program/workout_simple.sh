...
if [ -z "${TISSLM_SKIP_COMPILE:-}" ]; then
    ...
    echo "=== Skipping TissDB Compilation (using existing tissdb_exe) ==="
fi
...
if [ -z "${TISSLM_SKIP_COMPILE:-}" ]; then
    ...
    echo "=== Skipping Model Compilation (using existing train_model_exe) ==="
fi

############################################
echo "=== Run Training ==="
############################################
# Removed pipe to cat to minimize buffering layers
./train_model_exe 
