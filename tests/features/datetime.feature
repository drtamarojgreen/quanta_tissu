Feature: Date and Time Data Types
  ...
  Scenario: Insert and select a DATE value
    ...
    When I execute the TissQL query "INSERT INTO events (event_name, event_date) VALUES ('New Year', DATE '2024-01-01')"
    When I execute the TissQL query "SELECT event_name, event_date FROM events WHERE event_name = 'New Year'"
  ...
  Scenario: Insert and select a TIME value
    ...
    When I execute the TissQL query "INSERT INTO appointments (title, app_time) VALUES ('Lunch', TIME '12:30:00')"
    When I execute the TissQL query "SELECT title, app_time FROM appointments WHERE title = 'Lunch'"
    Then the query result should have 1 document
  ...
  Scenario: Insert and select a DATETIME value
    ...
    When I execute the TissQL query "INSERT INTO logs (level, log_time) VALUES ('INFO', DATETIME '2024-07-26 10:00:00')"
    When I execute the TissQL query "SELECT level, log_time FROM logs WHERE level = 'INFO'"
    Then the query result should have 1 document
  ...
  Scenario: Filter records using a WHERE clause with DATE
    ...
    When I execute the TissQL query "INSERT INTO history (event, event_date) VALUES ('Event A', DATE '2023-12-31')"
    When I execute the TissQL query "INSERT INTO history (event, event_date) VALUES ('Event B', DATE '2024-01-15')"
    When I execute the TissQL query "INSERT INTO history (event, event_date) VALUES ('Event C', DATE '2024-02-01')"
    When I execute the TissQL query "SELECT event FROM history WHERE event_date > DATE '2024-01-01'"
